...
impl MeshNodeBuilder {
    ...
    // Note: a `.relay_auth(url, token)` method for gated iroh relays
    // lives on the separate `--relay-auth` PR. It will be added back
    // here once that lands so the SDK can carry per-relay bearer
    // tokens.
