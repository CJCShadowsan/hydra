...
fn docker_check() -> DoctorCheck {
    ...
    DoctorCheck {
        ...
        ok: ready,
        detail: match (installed, ready) {
            (false, _) => "docker CLI not found on PATH".to_string(),
            (true, true) => "docker daemon is reachable".to_string(),
            (true, false) => "docker CLI found, but daemon is not reachable".to_string(),
        },
    }
}
