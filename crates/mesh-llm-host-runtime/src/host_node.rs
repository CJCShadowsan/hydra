...
pub struct MeshServeSpec {
    ...
    pub relays: Vec<String>,
    /// Per-relay bearer tokens for gated iroh relays. Maps to
    /// repeatable `--relay-auth URL=TOKEN`.
    pub relay_auths: HashMap<String, String>,
...
impl MeshServeSpec {
    ...
    fn append_relay_args(&self, argv: &mut Vec<std::ffi::OsString>) {
        ...
        for (url, token) in &self.relay_auths {
            argv.push("--relay-auth".into());
            argv.push(format!("{url}={token}").into());
        }
