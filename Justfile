...
# Run the UI dev server proxying to the public hydra-llm.cloud API
ui-dev-public: (ui-dev "https://hydra-llm.cloud")
...
test-all:
    ...
    if curl -sf http://127.0.0.1:3131/health >/dev/null 2>&1; then
        ...
        MESH_UI_API_ORIGIN="https://hydra-llm.cloud" VITE_API_URL="https://hydra-llm.cloud" bash -c 'cd "{{ ui_dir }}" && pnpm exec vite --host 0.0.0.0 --port 5173' &
        DEV_PID=$!
