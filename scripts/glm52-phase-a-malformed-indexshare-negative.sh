...
OUT_DIR="${OUT_DIR:-/tmp/glm52-phase-a-malformed-indexshare-negative}"
EXPECTED_ERROR="GLM-DSA top-k sideband payload size is not token-major i32"

usage() {
  cat <<'EOF'
Usage: scripts/glm52-phase-a-malformed-indexshare-negative.sh [options]

Proves the native GLM-5.2 runtime contract rejects a Shared GLM-DSA layer slice
when the top-k sideband flag is present but the sideband payload is malformed.
...
set +e
SKIPPY_BENCH_GLM_DSA_MALFORMED_TOP_K_BYTES=1 \
...
if [[ "$rc" == "0" ]]; then
  echo "malformed IndexShare negative unexpectedly succeeded" >&2
...
if ! grep -Fq "$EXPECTED_ERROR" "$OUT_DIR/stderr.txt"; then
  echo "malformed IndexShare negative failed for the wrong reason" >&2
...
summary = {
    "passed": True,
    "phase": "A",
    "scope": "native GLM-5.2 runtime contract negative: Shared layer rejects malformed IndexShare/top-k sideband",
    "layer_start": 3,
    "layer_end": 4,
    "return_code": rc,
    "expected_error": expected_error,
    "malformed_top_k_bytes": 1,
...
echo "GLM-5.2 Phase-A malformed IndexShare negative passed"
