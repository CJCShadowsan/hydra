...
OUT_DIR="${OUT_DIR:-/tmp/glm52-phase-a-missing-indexshare-negative}"
EXPECTED_ERROR="GLM-DSA consumer slices require top-k sideband input"

usage() {
  cat <<'EOF'
Usage: scripts/glm52-phase-a-missing-indexshare-negative.sh [options]

Proves the native GLM-5.2 runtime contract rejects a Shared GLM-DSA layer slice
when the required IndexShare/top-k sideband is missing.
...
if [[ "$rc" == "0" ]]; then
  echo "missing IndexShare negative unexpectedly succeeded" >&2
...
if ! grep -Fq "$EXPECTED_ERROR" "$OUT_DIR/stderr.txt"; then
  echo "missing IndexShare negative failed for the wrong reason" >&2
...
summary = {
    "passed": True,
    "phase": "A",
    "scope": "native GLM-5.2 runtime contract negative: Shared layer rejects missing IndexShare/top-k sideband",
    "layer_start": 3,
    "layer_end": 4,
    "return_code": rc,
...
echo "GLM-5.2 Phase-A missing IndexShare negative passed"
