...
OUT_DIR="${OUT_DIR:-/tmp/glm52-phase-a-real-warmup-wrong-width-indexshare-negative}"
EXPECTED_ERROR="GLM-DSA top-k sideband width does not match expected IndexShare width"

usage() {
  cat <<'EOF'
Usage: scripts/glm52-phase-a-real-warmup-wrong-width-indexshare-negative.sh [options]

Proves the native GLM-5.2 runtime contract rejects a Shared GLM-DSA layer slice
after the target stage has been warmed through a real Full-layer IndexShare
producer. The measured activation sideband is token-major i32 but too narrow for
the warmed target session.
...
set +e
SKIPPY_BENCH_GLM_DSA_REAL_TOP_K_WARMUP_SOURCE_LAYER_START=2 \
...
if [[ "$rc" == "0" ]]; then
  echo "real-warmup wrong-width IndexShare negative unexpectedly succeeded" >&2
...
if ! grep -Fq "$EXPECTED_ERROR" "$OUT_DIR/stderr.txt"; then
  echo "real-warmup wrong-width IndexShare negative failed for the wrong reason" >&2
  echo "expected: $EXPECTED_ERROR" >&2
  echo "stderr=$OUT_DIR/stderr.txt" >&2
  tail -120 "$OUT_DIR/stderr.txt" >&2 || true
  exit 1
fi
...
summary = {
    "passed": True,
    "phase": "A",
    "scope": "native GLM-5.2 runtime contract negative: real-producer-warmed Shared layer rejects wrong-width IndexShare/top-k sideband",
    "warmup_source_layer_start": 2,
    "layer_start": 3,
    "layer_end": 4,
    "position_start": 64,
    "kv_warmup_tokens": 64,
...
echo "GLM-5.2 Phase-A real-warmup wrong-width IndexShare negative passed"
