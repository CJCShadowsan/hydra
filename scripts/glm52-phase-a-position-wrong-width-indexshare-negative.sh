...
OUT_DIR="${OUT_DIR:-/tmp/glm52-phase-a-position-wrong-width-indexshare-negative}"
EXPECTED_ERROR="GLM-DSA top-k sideband width does not match expected IndexShare width"

usage() {
  cat <<'EOF'
Usage: scripts/glm52-phase-a-position-wrong-width-indexshare-negative.sh [options]

Proves the native GLM-5.2 runtime contract rejects a Shared GLM-DSA layer slice
when the top-k sideband is token-major i32 but too narrow for the current
session position. This guards the IndexShare width contract against accidentally
using the decode-start sideband width for all positions.
...
if [[ "$rc" == "0" ]]; then
  echo "position wrong-width IndexShare negative unexpectedly succeeded" >&2
...
if ! grep -Fq "$EXPECTED_ERROR" "$OUT_DIR/stderr.txt"; then
  echo "position wrong-width IndexShare negative failed for the wrong reason" >&2
...
summary = {
    "passed": True,
    "phase": "A",
    "scope": "native GLM-5.2 runtime contract negative: positioned Shared layer rejects wrong-width IndexShare/top-k sideband",
    "layer_start": 3,
    "layer_end": 4,
    "position_start": 64,
    "return_code": rc,
...
echo "GLM-5.2 Phase-A position wrong-width IndexShare negative passed"
