...
- If `[runtime] reconcile_model_targets = true` is enabled, unserved local explicit interests that are already present on disk and fit the current node may be runtime-loaded automatically. An already-serving host may also replace a lower-demand local model with a locally present, higher-demand unserved target. Leave it unset for read-only advisory checks.
