...
- POSIX-mounted DataSpace: use `hydra placement prefetch ... --posix-root /vast/...`.
...
not hard-code undocumented VAST-only APIs. Hydra publishes the artifact into
...
curl -X POST http://127.0.0.1:3131/api/placement/prefetch \
  ...
  -d '{
    ...
    "provider": { "type": "posix", "root": "/vast/global/hydra" },
    "vast_trigger": {
      "mode": "data_engine_webhook",
      "endpoint": "https://vast-dataengine.example.internal/hydra/ship",
      "tenant": "acme-ai",
      "dataspace": "prod-dataspace",
      "source_namespace": "/vast/global/hydra",
      "destination_namespace": "/vast/site-b/hydra",
      "target_sites": ["site-b"]
    }
  }'
```
...
If the namespace publish succeeds but the trigger endpoint fails, Hydra records
...
[placement]
provider = "posix"
posix_root = "/vast/global/hydra"
vast_trigger_endpoint = "https://vast-dataengine.example.internal/hydra/ship"
vast_tenant = "acme-ai"
vast_dataspace = "prod-dataspace"
vast_source_namespace = "/vast/global/hydra"
vast_destination_namespace = "/vast/site-b/hydra"
