...
`docs/CNAME`, `docs/install.sh`, `docs/install.ps1`, `docs/mesh-llm-logo.svg`,
...
and the repo-root `install.sh` / `install.ps1` into `docs/` for deployment.
...
- The Hydra website at hydra-llm.cloud is deployed from `website/`. Breaking
  website changes that pass local checks can still break the public docs and
  install surface for real users.
